"""E2 DISCRIMINATION GATE -- minimum-length CONTAINMENT objective.

Question: does adding geometric structure (more regions K) measurably improve a
minimum-length CONTAINMENT objective? We require containment fraction
(total_edep / E_beam) >= ``contain_target`` while paying a per-layer LENGTH cost
for material, and for a fixed inner-iteration budget we optimize per-region
absorber/gap thicknesses for K in {1,2,4,...}. More regions = more knobs at the
SAME iteration budget -> a fair test of whether structure helps. Because the
length cost is paid per layer while containment gradient is concentrated in the
front (shower) layers, the optimum is NON-UNIFORM (thick front, thin tail), which
structure can exploit and a single uniform region cannot.

VERDICT: DISCRIMINATES if final containment loss decreases clearly and roughly
...
from tools.optimizer import optimize_inner_containment
from tools.containment_target import containment, containment_loss
...
_SUMMARY_JSON = _HERE / "e2_discriminate_containment_summary.json"
...
def parse_args(argv=None):
    p = argparse.ArgumentParser(
        description="E2 containment discrimination gate (incremental + resumable).")
    ...
    p.add_argument("--n-events", type=int, default=500,
                   help="events per containment measurement (default 500)")
    ...
    p.add_argument("--contain-target", type=float, default=0.5,
                   help="target containment fraction total_edep/E_beam (default 0.5)")
    p.add_argument("--lam", type=float, default=1e4,
                   help="containment penalty weight lambda (default 1e4)")
    p.add_argument("--mu", type=float, default=1.0,
                   help="length-cost weight mu (default 1.0)")
    p.add_argument("--e-beam", type=float, default=10000.0,
                   help="beam energy E_beam [MeV] for the containment fraction "
                        "(default 10000)")
    p.add_argument("--out-jsonl", type=str,
                   default=str(_HERE / "e2_discriminate_containment_results.jsonl"),
                   help="JSONL checkpoint file (default "
                        "experiments/e2_discriminate_containment_results.jsonl)")
...
def run_unit(n_layers, K, contain_target, e_beam, lam, mu,
    ...
    init_total, init_frac = containment(
        rep.to_design_point(), n_events, seeds, ctrl=ctrl)
    init_loss = float(containment_loss(
        init_total, contain_target, e_beam, lam, mu, rep.regions))

    res = optimize_inner_containment(
        rep, contain_target, e_beam, lam, mu, constraints=CONSTRAINTS,
        max_iters=max_iters, n_events=n_events, seeds=seeds, ctrl=ctrl,
        trust_region=TRUST_REGION)

    final_loss = float(res.final_objective)
    final_frac = float("nan")
    if res.history:
        final_frac = float(res.history[-1].get("containment_frac", float("nan")))
    ...
    return {
        ...
        "final_containment_frac": final_frac,
        "final_absorber_mm": final_abs,
        "final_gap_mm": final_gap,
        "initial_uniform_loss": init_loss,
        "initial_containment_frac": float(init_frac),
        "contain_target": float(contain_target),
        "lam": float(lam),
        "mu": float(mu),
        "e_beam": float(e_beam),
...
def final_verdict(out_path, n_layers, k_list, robust_layers):
    ...
    print(f"{'K':>4} {'final_loss':>14} {'ratio_vs_K1':>14} {'frac':>8}")
    for t in table:
        fr = prim[t["K"]].get("final_containment_frac", float("nan"))
...
def main(argv=None):
    ...
    for nl, K in to_run:
        row = run_unit(nl, K, args.contain_target, args.e_beam,
                       args.lam, args.mu, args.n_events,
                       args.max_iters, seeds, ctrl)
        append_row(out_path, row)
        print(f"[done] nl={nl} K={K} n_regions={row['n_regions']} "
              f"final_loss={row['final_loss']:.6f} "
              f"final_frac={row['final_containment_frac']:.4f} "
