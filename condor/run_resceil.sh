#!/bin/bash
# HTCondor worker wrapper for the VALUE-ONLY resolution-ceiling experiment.
#
...
#
# All experiment args (--design, --seed, --n-layers, --n-events, --out-jsonl)
# are forwarded verbatim, so the single submit file drives both the tiny test
# and the real per-(design,seed) runs.
set -euo pipefail
...
# --- run ------------------------------------------------------------------
echo "[wrapper] launching: python -u -m experiments.resolution_ceiling $*"
exec python -u -m experiments.resolution_ceiling "$@"
