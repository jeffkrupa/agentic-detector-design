#!/bin/bash
# HTCondor worker wrapper for the GRADIENT-RELIABILITY measurement.
#
...
#   3. exec's the grad-reliability experiment module, passing through all args.
#
# All experiment args (--seed, --n-layers, --n-events, --out-jsonl, ...) are
# forwarded verbatim, so the single submit file drives both the tiny smoke test
# and the real high-statistics per-seed runs.
set -euo pipefail
...
# --- run ------------------------------------------------------------------
echo "[wrapper] launching: python -u -m experiments.grad_reliability $*"
exec python -u -m experiments.grad_reliability "$@"
