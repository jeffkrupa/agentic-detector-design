#!/bin/bash
# HTCondor worker wrapper for the wave-9 knob validation (fidelity loop).
#
# Mirrors run_wave2_validation.sh: sources the project venv by absolute path,
...
# unit of fidelity/wave9_validation.py, idempotent via the sim cache and
# per-unit JSONL files.
#
# Canonical AFS copy (the .sub executable) lives at
#   /afs/cern.ch/user/j/jekrupa/condor_bin/run_wave9_validation.sh
...
# Sanity: the wave-9 KNOB binary (build_agent_fwd, branch
# knob/dot-caps @ fc388aa) + data must be reachable from here.
...
# --- run ------------------------------------------------------------------
echo "[wrapper] launching: python -u -m fidelity.wave9_validation $*"
exec python -u -m fidelity.wave9_validation "$@"
