"""Wave-11 condor validation, pre-registered item C: the accumulated-state dot
governor (`--track-dot-cap 1000:50`) as the absorber-channel severing
replacement, at scale on-design and off-design against its own FD arms.

Validates the wave-10 knob branch `knob/dot-governor` @ 3b6d45f (hepemshow,
built in the -gov WORKTREE /eos/user/j/jeffkrup/agentic/hepemshow-gov/
build_gov_fwd/ -- do NOT rebuild) against the standing 1M-event FD truth
...
Four configurations, 40 jobs / 800k events total, 20000 events each
(n=1000 od timing smoke projected ~3100 s per 20k unit -- well inside the
10800 s in-process timeout, so no sub-job splitting needed):

    abs_gov_val       s1-10  baseline design (a=2.30 g=5.70), unsevered
                             -x 0 -y 0 -B 0, --track-dot-cap 1000:50,
                             absorber seed (-a 2.3:1)
    abs_gov_od        s1-10  off-design a=3.00 (g=5.70), unsevered +
                             governor, absorber seed (-a 3.0:1)
    abs_fd_od_plus    s1-10  a=3.1, CANONICAL severed flags (primal-only FD
    abs_fd_od_minus   s1-10  a=2.9,  arm; dot slot energy-seeded per repo
...
same trick as waves 1-9; global config.yaml untouched).

Per-event capture (AD configs only): the run sets HEPEMSHOW_EVENT_DUMP to a
local temp path (env-gated telemetry inherited from knob/dot-caps;
primal-identical, wave-10 gates G1/G2), then reduces the dump to per-event
core-window sums W_e and layer-sum totals T_e saved as
fidelity/wave11_runs/<config>_s<seed>_events.npz. A cache-hit rerun cannot
...
Run one unit (one condor job):
    python -u -m fidelity.wave11_absval --config abs_gov_val --seed 1

    Appends one JSONL row (per-layer mean_E/var_E/mean_dE/var_dE, flags,
    binary provenance, per-event summary, wall time) to
    fidelity/wave11_runs/<config>_s<seed>.jsonl; skips the sim if an ok
    row with the same n_events already exists.

Aggregate after all 40 jobs finish:
    python -u -m fidelity.wave11_absval --analyze
        -> fidelity/wave11_absval_summary.json
    (With zero unit rows present it reports and exits WITHOUT writing a
    placeholder summary file.)

Pre-registered tests (ledger.jsonl wave-11 predicted_signature, written
before this batch; LEDGER.md wave-10 preview):
  1. abs_gov_val core-window (L5-18) mean vs 1M FD truth 2233.7 +- 14.8:
     within 2 sigma?  Seed-scatter AND pooled per-event SEs both reported;
     median / tm5 next to the wave-10 preview (pooled 2417.7 +- 194.0,
     medians 2214/1953); variance ratio vs the canonical severed absorber
     reference (experiments/perlayer_adfd/absorber_ad_s*.jsonl ok rows,
     same covariance-free var_dE window construction on both sides).
  2. abs_gov_od core mean vs its own FD: per-layer FD = (E+ - E-)/0.2 from
     the a=3.1/2.9 arms, UNPAIRED, seed-scatter errors (PROTOCOL: CRN
     pairing is worthless here) -- within 2 sigma?  The od FD value itself
     is reported (no prior truth exists at a=3.0).
  3. Layer-sum totals for ALL configs. The governor totals-bias is unknown
     (unlike the boundary cap's measured ~65% truncation) -- quantified
     here vs the on-design FD-truth total 1050.8 +- 20.2 and the od FD-arm
     total. Reported, not headlined (PROTOCOL: absorber totals are
...
# baseline; wave-10 governor build lives in the -gov worktree). Deliberately
# not in config.yaml; injected via the in-process config override in
# _wave_config(). The branch/SHA below are the truth for the binary (verified
# via strings + 10-event smoke with --track-dot-cap 1000:50 before
# submission; worktree HEAD checked at 3b6d45f).
KNOB_BINARY = ("/eos/user/j/jeffkrup/agentic/hepemshow-gov/"
               "build_gov_fwd/HepEmShow")
KNOB_BRANCH = "knob/dot-governor"
KNOB_SHA = "3b6d45f"

GOVERNOR_ARGS = ["--track-dot-cap", "1000:50"]

# name: (seeded_param, a_mm_override, severed, extra_args, seeds, per_event)
CONFIGS = {
    "abs_gov_val":     ("a", None, False, GOVERNOR_ARGS,
                        list(range(1, 11)), True),
    "abs_gov_od":      ("a", 3.0, False, GOVERNOR_ARGS,
                        list(range(1, 11)), True),
    "abs_fd_od_plus":  ("energy", 3.1, True, [],
                        list(range(1, 11)), False),
    "abs_fd_od_minus": ("energy", 2.9, True, [],
                        list(range(1, 11)), False),
}
FD_OD_DENOM_MM = 0.2  # (a=3.1) - (a=2.9), PROTOCOL large-h methodology
...
TRUTH_ABS_TOTAL = (1050.8, 20.2)     # 1M FD, sum L0-49, on-design
PREVIEW_GOV = {                      # n=2000x2 preview, LEDGER wave-10
    "grid_point": "P1000:E50",
    "pooled_core_mean": 2417.7, "pooled_core_se": 194.0,
    "ratio_vs_truth": 1.082, "z_vs_truth": 0.95,
    "median_s1": 2214.0, "median_s2": 1953.0,
    "varWe_ratio_s1": 74.8, "varWe_ratio_s2": 270.7,
    "canonical_varWe_s1": 2.4e6, "canonical_varWe_s2": 4.5e5,
}

_HERE = Path(__file__).resolve().parent
_REPO = _HERE.parent
OUT_DIR = _HERE / "wave11_runs"
SUMMARY_PATH = _HERE / "wave11_absval_summary.json"
FD_TRUTH_PATH = _REPO / "experiments" / "perlayer_adfd_1M_summary.json"
CANONICAL_ABS_UNIT_GLOB = str(_REPO / "experiments" / "perlayer_adfd"
                              / "absorber_ad_s*.jsonl")
...
def _parse_event_dump(path: Path, n_layers: int):
    ...
    n_layers dots (same parser as fidelity/wave9_validation.py)."""
    lo, hi = CORE_WINDOW
    W, T = [], []
    with open(path) as fh:
        for line in fh:
...
def run_unit(config: str, seed: int, n_events: int) -> int:
    ...
    dump_dir = tempfile.mkdtemp(prefix="wave11av_dump_") if want_events else None
...
def _canonical_var_window(n_events: int, lo: int, hi: int):
    """Seed-averaged per-event var_dE from the canonical severed absorber
    units, summed over [lo, hi] (covariance-free construction; the knob-on
    side of the ratio uses the same construction). Returns (window_sum,
    n_seeds)."""
    rows = [r for r in _load_unit_rows(CANONICAL_ABS_UNIT_GLOB, n_events)
...
def analyze(n_events: int, out_json: Path) -> int:
    ...
    summary = {
        "wave": 11,
        "item": "C (governed-absorber validation)",
        "knob_branch": KNOB_BRANCH,
        "knob_sha": KNOB_SHA,
        "governor": " ".join(GOVERNOR_ARGS),
    ...
    per_cfg = {}
    complete = True
    for cfg_name, (_, a_ovr, severed, extra, exp_seeds, want_ev) in CONFIGS.items():
    ...
    # -- Test 1: abs_gov_val core window vs 1M FD truth ---------------------- #
    t1 = {"description": f"abs_gov_val core L{lo}-{hi} mean vs 1M FD truth "
                         f"{TRUTH_ABS_CORE[0]} +- {TRUTH_ABS_CORE[1]}; "
                         f"pre-registered: within 2 sigma. Seed-scatter AND "
                         f"pooled per-event SEs; median/tm5 vs the wave-10 "
                         f"preview; var ratio vs canonical severed absorber "
                         f"(covariance-free var_dE window, both sides); "
                         f"per-layer profile.",
          "preview_reference": PREVIEW_GOV}
    fd_w, fd_w_se = _fd_window(abs_truth, lo, hi)
    t1["fd_truth_window"] = [fd_w, fd_w_se]
    if per_cfg.get("abs_gov_val") is not None:
        mean_l, se_l, stack = per_cfg["abs_gov_val"]
        ad_w, ad_w_se = _window_sum(stack, lo, hi)
        z = _z(ad_w, ad_w_se, fd_w, fd_w_se)
        t1.update({"ad_core_mean_seed_scatter": ad_w,
                   "ad_core_se_seed_scatter": ad_w_se,
                   "z_seed_scatter_vs_truth": z,
                   "pass_within_2sigma_seed_scatter": bool(abs(z) <= 2.0),
                   "per_seed_core_means": stack[:, lo:hi + 1].sum(axis=1).tolist()})
        W, T, with_ev, without = _pooled_events(rows, "abs_gov_val")
        if W is not None:
            pe = _per_event_stats(W, T)
            pe["seeds_with_events"] = with_ev
            pe["seeds_without_events"] = without
            zp = _z(pe["mean_W"], pe["se_W"], fd_w, fd_w_se)
            pe["z_pooled_vs_truth"] = zp
            pe["pass_within_2sigma_pooled"] = (bool(abs(zp) <= 2.0)
        ...
        on_rows = [r for r in rows if r.get("config") == "abs_gov_val"
        ...
        fd = np.asarray(abs_truth["per_layer"]["fd"], dtype=float)
        fd_se = np.asarray(abs_truth["per_layer"]["fd_se"], dtype=float)
    ...
    summary["tests"]["1_abs_gov_core_vs_truth"] = t1

    # -- Test 2: off-design governed absorber vs its own FD ------------------ #
    t2 = {"description": f"abs_gov_od (a=3.0) core L{lo}-{hi} mean vs own FD "
                         f"(E+ - E-)/{FD_OD_DENOM_MM} from a=3.1/2.9 arms, "
                         f"UNPAIRED, seed-scatter errors; pre-registered: "
                         f"within 2 sigma. The od FD value is itself a "
                         f"reported result (no prior truth at a=3.0)."}
    have_arms = all(per_cfg.get(c) is not None
                    for c in ("abs_fd_od_plus", "abs_fd_od_minus"))
    if have_arms:
        parts = {}
        for c in ("abs_fd_od_plus", "abs_fd_od_minus"):
        ...
        (mp, sp, wp, wp_se, tp, tp_se) = parts["abs_fd_od_plus"]
        (mm, sm, wm, wm_se, tm, tm_se) = parts["abs_fd_od_minus"]
        ...
        t2["fd_core"] = fd_core
        t2["fd_core_se"] = fd_core_se
        t2["fd_total"] = (tp - tm) / FD_OD_DENOM_MM
        t2["fd_total_se"] = math.sqrt(tp_se ** 2 + tm_se ** 2) / FD_OD_DENOM_MM
        t2["fd_per_layer"] = fd_l.tolist()
        t2["fd_per_layer_se"] = fd_l_se.tolist()
    else:
        t2["fd_status"] = "MISSING FD ARM DATA"
    if per_cfg.get("abs_gov_od") is not None and have_arms:
        mean_l, se_l, stack = per_cfg["abs_gov_od"]
        ad_w, ad_w_se = _window_sum(stack, lo, hi)
        z = _z(ad_w, ad_w_se, fd_core, fd_core_se)
        ratio_l = np.where(fd_l != 0, mean_l / np.where(fd_l != 0, fd_l, 1.0),
                           np.nan)
        t2.update({"ad_core_mean": ad_w, "ad_core_se": ad_w_se,
        ...
        W, T, with_ev, without = _pooled_events(rows, "abs_gov_od")
        if W is not None:
            pe = _per_event_stats(W, T)
            pe["seeds_with_events"] = with_ev
            pe["seeds_without_events"] = without
            pe["z_pooled_vs_own_fd"] = _z(pe["mean_W"], pe["se_W"],
                                          fd_core, fd_core_se)
            t2["pooled_per_event"] = pe
    elif per_cfg.get("abs_gov_od") is None:
        t2["status"] = "MISSING AD DATA"
    summary["tests"]["2_abs_gov_offdesign_vs_own_fd"] = t2

    # -- Test 3: layer-sum totals (governor totals-bias, quantified) --------- #
    t3 = {"description": "layer-sum (L0-49) totals for all configs. The "
                         "governor totals-bias is UNKNOWN (the boundary cap "
                         "truncated ~65% of net total mass; the governor "
                         "clamps state carriers, not crossing generators, so "
                         "its totals behavior is a measurement, not a "
                         "prediction). Quantified vs the on-design FD-truth "
                         f"total {TRUTH_ABS_TOTAL[0]} +- {TRUTH_ABS_TOTAL[1]} "
                         "and the od FD-arm total. Reported, not headlined "
                         "(PROTOCOL: absorber totals are tail-noise-"
                         "dominated).",
          "fd_truth_abs_total_on_design": list(TRUTH_ABS_TOTAL)}
    for cfg_name in CONFIGS:
        key = "mean_E" if cfg_name.startswith("abs_fd_od") else "mean_dE"
        t3[cfg_name] = _totals_block(rows, cfg_name, key) or {"status": "MISSING"}
    tb = t3.get("abs_gov_val")
    if tb and "mean" in tb:
        r, r_se = _ratio(tb["mean"], tb["se"], TRUTH_ABS_TOTAL[0],
                         TRUTH_ABS_TOTAL[1])
        t3["gov_total_over_truth_on_design"] = [r, r_se]
    if "fd_total" in t2:
        t3["fd_od_total_derivative"] = [t2["fd_total"], t2["fd_total_se"]]
        tb_od = t3.get("abs_gov_od")
        if tb_od and "mean" in tb_od and t2["fd_total"]:
            r, r_se = _ratio(tb_od["mean"], tb_od["se"],
                             t2["fd_total"], t2["fd_total_se"])
            t3["gov_total_over_fd_off_design"] = [r, r_se]
    summary["tests"]["3_layer_sum_totals"] = t3
